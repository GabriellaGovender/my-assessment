...
def question_3():    
    ...
    # Find the `ApprovalPercentage` of loans by `LoanTerm` [Approved]

    qry = """SELECT LoanTerm,
             SUM(CASE WHEN ApprovalStatus='Approved' THEN 1 ELSE 0 END)/Count(LoanTerm)*100 AS ApprovalPercentage
             FROM loans 
             GROUP BY LoanTerm"""
...
def question_4():    
    ...
    qry = """SELECT CustomerClass, Count(CustomerClass) as Count FROM credit GROUP BY CustomerClass"""
...
def question_5():    
    ...
    qry = """UPDATE credit SET CustomerClass = 'C' WHERE CreditScore BETWEEN 600 AND 650; SELECT * FROM credit"""
